...
router.get("/:code", async  (req, res, next)=>{
    try {
        ...
        if (compResult.rows.length === 0) throw new ExpressError(`No such company: ${code}`, 404)
...
router.put("/:code", async  (req, res, next)=>{
    try {
        ...
        if (result.rows.length === 0) throw new ExpressError(`No such company: ${code}`, 404)
        else return res.json({"company": result.rows[0]})
    }
    catch (err) {return next(err)}
})
